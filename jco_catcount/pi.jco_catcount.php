...
class Jco_catcount {
	...
	*/
	public function Count_catitems()
	{
		...
		// check each site_id in array
		foreach ($site as $value)
		{
			if (is_numeric($value))
			{
				if(!$this->_site_exists($value))
				{
					$this->EE->TMPL->log_item(str_repeat("&nbsp;", 5) . "- JCO CATCOUNT ERROR: there is no site with an id of \"".$value."\" in your database");
					return FALSE;
				}
			}
			else
			{
				$this->EE->TMPL->log_item(str_repeat("&nbsp;", 5) . "- JCO CATCOUNT ERROR: site_id parameter \"".$value."\" is not a number");
				return FALSE;
			}
		}
		...
		if ($channel != "")
		{
			...
			{
				if(!$this->_channel_name_exists($value))
				{
					$this->EE->TMPL->log_item(str_repeat("&nbsp;", 5) . "- JCO CATCOUNT ERROR: there is no channel with a name of \"".$value."\" in your database");
					return FALSE;
				}
			}
		}
	...
	* ------------------------------------------------------------ */
	...
	/**
	* Check if category_id exists in DB
	*
	* @access	private
	* @return	boolean
	*/
	...
	*/
	...
	/**
	* Check if channel_name exists in DB
	*
	* @access	private
	* @return	boolean
	*/
	...
	* ------------------------------------------------------------ */
	...
	function usage()
	{
		...
		?>
			...
			site_id="1|2" : Optional (for MSM sites)
			The id of the sites you want to check
			Defaults to the id of the current site
			You can use piped site_ids like site_id="1|2"
